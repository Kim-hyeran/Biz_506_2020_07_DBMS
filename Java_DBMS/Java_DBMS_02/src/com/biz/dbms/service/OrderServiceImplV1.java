...
public class OrderServiceImplV1 implements OrderService {
	 ...
	 * throws Exception
	 * 
	 * 그동안 try-catch를 사용하여 자체적으로 exception을 handling(처리)하였으나,
	 * 	코드 내에 try-catch문이 많아지면 가독성이 매우 떨어진다.
	 * exception 처리를 직접 하지 않고, 이 method를 호출한 곳에 exception을 대리 요청한다. 
	 */
	@Override
	public int insert(OrderVO orderVO) throws SQLException {
	//insert를 호출하는 곳에서는 OrderVO를 주문정보 데이터를 담고 매개변수로 전달해야 하며,
	//try-catch로 exception 처리를 하고, 결과값으로 정수값을 받아서 insert유무 처리
		String sql=DBContract.ORDER_INSERT;
		
		PreparedStatement pSt=this.dbConn.prepareStatement(sql);
		
		//VO를 통해서 전달받은 데이터를 SQL 명령문과 합성
	...
	/*
	 * selectAll() method를 호출하는 곳에서는 아무런 매개변수 없이 method를 호출하고 exception 처리를 수행해야 하며
	 * return 값을 List<OrderVO>형 변수에 받아서 처리한다.
	 */
	@Override
	...
	/*
	 * selectAll()과 findBySeq() method 사이에서 ResultSet으로부터 데이터를 getter하여
	 * 	OrderVO에 setter하는 부분을 별도 method로 분리하고, selectAll()과 findBySeq()에서 호출하여 사용할 수 있도록 구현
	 */
	...
	private OrderVO setOrderVO(ResultSet rSet) throws SQLException {
		...
		//문자열.trim() : 문자열 앞뒤에 존재하는 공백 제거 method
	...
	public OrderVO findBySeq(long seq) throws SQLException {
		...
		//rSet에 담기게 될 table의 데이터는 단 1개의 데이터만 담기거나, 없는 상태가 된다.
		ResultSet rSet=pSt.executeQuery();
		if(rSet.next()) {
			OrderVO orderVO=this.setOrderVO(rSet);
			return orderVO;
